...
#define __GAME_H__
//<MOD>
#define WEIGHT_OLDJUMPNEWJUMP 5
#define JUMP_AGING_CONSTANT 0.000000314f
#define JUMP_MULTIPLIER 200.0f
...
{
    //<MOD>
    { "TURMALIN",  M_TEAM, "Team Deathmatch: Conquer the Brickxflag upon heigh." },
    //</MOD>
...
struct fpsent : dynent, fpsstate
{
    ...
    vec currentJumpDir;
    float currentJumpAge;
    float jumpStrength; // Goes down along a inverse logarithmic function
    ...
    //<MOD>
    void updatePool(fpsent *p)
    {
        if (p->EnergyPool <= 99.0f) p->EnergyPool++;
    }

    void addNewJump(fpsent *p,float strength)
    { //getting the players direction
        vec *temp= new vec( p->pitch*strength, p->roll*strength, p->yaw*strength);
      //weighted multiplication with the current cumpVector

          if (currentJumpAge > 0.01f)
          {
          p->currentJumpDir=p->currentJumpDir.mul(p->currentJumpAge* p->jumpStrength).add(*temp);
          p->currentJumpAge=(p->currentJumpAge+WEIGHT_OLDJUMPNEWJUMP)/WEIGHT_OLDJUMPNEWJUMP+1;
          p->currentJumpDir=p->currentJumpDir.normalize();
          }
              else
              {
               p->currentJumpDir[0]=p->pitch;
               p->currentJumpDir[1]=p->yaw;
               p->currentJumpDir[2]=p->roll;
              }
          p->jumpStrength=strength;
    }

    //this function transfers the jump to the physentsubclass
    void applyCurrentJump(fpsent * p)
    {

        p->currentJumpAge-=JUMP_AGING_CONSTANT;

     if (p->currentJumpAge > 0.1f)
         {
             p->falling.mul(1-p->currentJumpAge);
             p->vel.add(p->currentJumpDir.mul(logf(p->currentJumpAge+1)*JUMP_MULTIPLIER));
         }
    else  //neutralizing the jump altogether
        {
            p->currentJumpAge=0;
            p->currentJumpDir.mul(0.0f);
    ...
    void respawn()
    {
        ...
        currentJumpDir[1]=0.0f;currentJumpDir[2]=0.0f;currentJumpDir[3]=0.0f;currentJumpDir[4]=1.0f;
        //</MOD>
    }
};
