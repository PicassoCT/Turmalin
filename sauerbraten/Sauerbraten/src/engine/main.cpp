...
#define CHARGEPERMILLISECOND 500
//</MOD>
    ...
    while(events.length() || pollevent(event))
    {
        ...
        switch(event.type)
        {
            ...
            case SDL_MOUSEBUTTONDOWN:
                   //<MOD>MOUSEBUTTON  TODO needs to be more specific if left or right
                   {
                     if (((event.button.type & MOUSEEVENTF_RIGHTDOWN) >0)&&((event.button.state& SDL_PRESSED)>0) && (curtime-game::player1->jLastMillis !=0)  )
                     {
                     game::player1->jChargeMillis+=curtime-game::player1->jLastMillis;
                     game::player1->jLastMillis=curtime;
                     }
            ...
            case SDL_MOUSEBUTTONUP:
                 //<MOD>  TODO: Check if it is left or right mousebutton that goes UP
                 if (((event.button.type& MOUSEEVENTF_RIGHTUP) > 0) && ((event.button.state&SDL_RELEASED)>0)&& (game::player1->EnergyPool != 0))
                 {
                     float strength=game::player1->jChargeMillis/CHARGEPERMILLISECOND;
                     if (strength > game::player1->EnergyPool) strength= floorf(game::player1->EnergyPool+0.5f);

                     if (strength > 1.0f)
                     {
                      game::player1->addNewJump(game::player1, strength);
                      game::player1->EnergyPool-=strength;
                     }
...
bool applyGameFrame()
{
            ...
            //check if currFlagPolePtr has a Player if yes

             //check if current Ptr has a cube combination, if not create a cube combination


             // check for Input - if yes


             //check if Input can be applied


             //if yes, apply input if no

             //check if there are motions to continue



             //check if collission with the KubeKlumpen are happening

             //if yes,transfer the CubeKombination
...
int main(int argc, char **argv)
{
    ...
    ignoremousemotion();
    //<MOD>
    Compound * SingleComp = new Compound();
    ...
    for(;;)
    {
        ...
        updatetime();
        //<MOD>
        SingleComp->update(frames,elapsed);
        //</MOD>
        checkinput();
        menuprocess();
        tryedit();
        ...
        if (game::players.length()>0)
        {
            loopv(game::players)
            {
             game::players[i]->updatePool(game::players[i]);
             game::players[i]->updatePool(game::players[i]);
            }
        }
